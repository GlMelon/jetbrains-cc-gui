...
**opencode serve 托管流式通道(`cli/opencode/serve/`,架构差异例外条款)**:opencode 在 one-shot 之外另有一条 `opencode serve` 长驻进程 + HTTP/SSE 通道(`OpenCodeServeManager` / `OpenCodeServeClient` / `OpenCodeServeSession`,默认开启,`-DcliPersistent.opencode.serve.enabled=false` 一键回切 one-shot)。该通道属「连接模型本身不同」的架构例外,横切项等价落实:serve stdout drain(独立守护线程)、extraEnv + gateway 注入(spawn 时 `buildBaseEnvironment` + `applyExtraEnv` + `buildCliConfig` env)、cwd null → home 回退(spawn cwd 与 `POST /session?directory=` 两处)、ProcessManager 注册与 dispose、B13 等价重试(prompt_async 4xx → 清 sessionId 建新会话重试一次)、指纹(CLI 版本 + gateway endpoint + 注入 env 内容)漂移 / 崩溃 / SSE 半开(client 侧 server.heartbeat 活性看门狗超时关流)重建 + 连续失败 2 次熔断。一点有意差异:interrupt 走 `POST /session/{id}/abort` 确定性取消(优于杀树——serve 为 project 级共享进程,杀树波及其他 tab;`CLI_INTERRUPT_FALLBACK_MS` 无回应仍 `terminateServe` 杀树兜底)。权限请求已落地交互式授权:非 bypass 模式的 permission.asked 经 `ServePermissionGate` 转发 `PermissionService.requestInteractivePermission`(决策记忆命中直接应答,否则前端对话框),用户决策映射 allow→`once` / allow_always→`always` / deny→`reject`,对话框超时 / 闸口异常 / 无 Project 路径一律保守 `reject`。SSE 断线当轮失败不重发(已递交轮防双发),prompt 递交前失败当轮降级 one-shot(对齐 acquire 未命中静默降级)。
