...
/**
 * Kimi CLI 会话:每个 Tab 独立实例,使用 {@code kimi run "<msg>" --format json}
 ...
 * <ul>
 *   <li>B1:不预创建 session。{@code kimi run} 隐式创建会话,sessionID 从事件流 [SESSION_ID] 提取。</li>
 ...
 */
public class KimiCliSession implements CliSession {

    private static final Logger LOG = Logger.getInstance(KimiCliSession.class);
    ...
    public KimiCliSession(String tabId) {
        this(tabId, null);
    }

    public KimiCliSession(String tabId, McpGatewayService gatewayService) {
        this.tabId = tabId;
    ...
    public CompletableFuture<Void> send(CliSendRequest request, CliSessionCallback callback) {
        ...
        return CliSessionExecutor.runAsync(() -> {
            ...
            try {
                LOG.info("[CliConcurrencyDiag][KimiCliSession] send task started"
                        + ": tabId=" + tabId
                ...
                try {
                    ...
                    LOG.info("[CliConcurrencyDiag][KimiCliSession] attachments prepared"
                            + ": tabId=" + tabId
                    ...
                    LOG.warn("[KimiCliSession][" + tabId + "] process attachments failed", e);
                    attachFiles = List.of();
                }
                ...
                if (retry) {
                    LOG.info("[CliConcurrencyDiag][KimiCliSession] continuation session invalidated; retrying as fresh turn"
                            + ": tabId=" + tabId
                            + ", elapsedMs=" + elapsedMillis(sendStartNanos)
                            + ", thread=" + Thread.currentThread().getName());
                    sessionId = null;
                ...
                LOG.warn("[KimiCliSession][" + tabId + "] send failed", e);
                if (wasInterrupted()) {
                    callback.onInterrupted(null, CliConstants.I18N_REQUEST_INTERRUPTED);
                } else {
                    String err = CliErrorFormatter.formatError("Kimi", e.getMessage());
    ...
    /**
     * 执行一次 {@code kimi run}。返回 true 表示发生了续接 session 失效、应作为首轮重试
    ...
    private boolean runOnce(
        ...
        LOG.info("[CliConcurrencyDiag][KimiCliSession] command prepared"
                + ": tabId=" + tabId
                + ", effectiveSessionId=" + effectiveSessionDisplay
                + ", elapsedMs=" + elapsedMillis(sendStartNanos)
                + ", thread=" + Thread.currentThread().getName());
        LOG.info("[KimiCliSession][" + tabId + "] Command: " + String.join(" ", cmd));
        ...
        if (request.cwd() != null && !request.cwd().isBlank()) {
            ...
            if (cwd.isDirectory()) {
                ...
                LOG.warn("[KimiCliSession][" + tabId + "] CWD does not exist, falling back to home: " + request.cwd());
        ...
        LOG.info("[CliConcurrencyDiag][KimiCliSession] starting process"
                + ": tabId=" + tabId
                + ", effectiveSessionId=" + effectiveSessionDisplay
                + ", elapsedMs=" + elapsedMillis(sendStartNanos)
                + ", thread=" + Thread.currentThread().getName());
        Process process = pb.start();
        LOG.info("[CliConcurrencyDiag][KimiCliSession] process started"
                + ": tabId=" + tabId
                + ", effectiveSessionId=" + effectiveSessionDisplay
                + ", elapsedMs=" + elapsedMillis(sendStartNanos)
                + ", thread=" + Thread.currentThread().getName());
        CliProcessHandle currentHandle = new CliProcessHandle(process, "kimi-tab-" + tabId);
                        ...
                        for (int i = 0; i < n; i++) {
                            ...
                            if (b == '\n') {
                                if (!firstOutputLogged) {
                                    ...
                                    LOG.info("[CliConcurrencyDiag][KimiCliSession] first stdout line buffer reached"
                                            + ": tabId=" + tabId
                                            + ", effectiveSessionId=" + effectiveSessionDisplay
                                            + ", elapsedMs=" + elapsedMillis(sendStartNanos)
                                            + ", thread=" + Thread.currentThread().getName());
        ...
        LOG.info("[CliConcurrencyDiag][KimiCliSession] process exited"
                + ": tabId=" + tabId
                + ", effectiveSessionId=" + effectiveSessionDisplay
                + ", exitCode=" + exitCode
                + ", elapsedMs=" + elapsedMillis(sendStartNanos)
        ...
        if (outcome.timedOut() && !wasInterrupted()) {
            String err = CliErrorFormatter.formatError("Kimi", "CLI request timed out");
            callback.onError(err);
        ...
        if (exitCode == 0 && !parser.hasError()) {
            if (!parser.streamEnded()) {
                if (isSilentEmptyFailure(parser)) {
                    String err = CliErrorFormatter.formatError("Kimi",
                            "进程退出但未返回任何内容(exit=0,无事件流)。"
                                    + "常见原因:子进程阻塞读 stdin,或 kimi/provider 内部错误。"
                                    + "请在命令行执行 kimi run 并重定向空 stdin 对照验证,检查 kimi 配置与 provider。");
                    callback.onError(err);
                    callback.onComplete(false, parser.accumulatedText(), err);
                    return false;
                }
        ...
        String err = errDiag.isBlank()
                ? CliErrorFormatter.formatExitError("Kimi", exitCode, diagnostic)
                : CliErrorFormatter.formatError("Kimi", errDiag);
        callback.onError(err);
        callback.onComplete(false, parser.accumulatedText(), err);
        return false;
        } finally {
    ...
    public void interrupt() {
        ...
        if (h != null) {
            ...
            h.interrupt();
            LOG.info("[TabPerf] KimiCliSession.interrupt returned in "
    ...
    /**
     * 构造 {@code kimi run "<msg>" --format json [-s sessionId] [能力 flags]}。
    ...
    List<String> buildRunCommand(CliSendRequest request, String effectiveSessionId, List<File> attachFiles) {
        String executable = KimiCliResolver.findExecutable();
        ...
        String variant = kimiVariant(request.reasoningEffort());
        if (variant != null) {
    ...
    /**
     * reasoningEffort → kimi --variant(设计 §7.2):
     * low→minimal, medium→省略(默认), high→high, xhigh/max→max。
     */
    private static String kimiVariant(String reasoningEffort) {
    ...
    private void processLine(CliOutputLimits.LineBuffer lineBuf, KimiCliStreamParser parser, StringBuilder diagnostic) {
     ...
     * 供 {@link ProcessBuilder#redirectInput} 在 OS 句柄层给 kimi 空 stdin(立即 EOF)。
     */
    ...
    /**
     * 判定是否"静默空失败":kimi 进程 exit0,但整轮未解析到任何有效事件
     * ({@code !receivedAnyEvent},即无 sessionID / 文本 / [STREAM_END])。
     * 此组合表明 kimi 未产出事件流(典型:阻塞读 stdin / 内部静默错误),
     * 应上报错误而非静默空完成。
     */
    static boolean isSilentEmptyFailure(KimiCliStreamParser parser) {
