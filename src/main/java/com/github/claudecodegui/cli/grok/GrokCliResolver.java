...
/**
 * Resolves the Grok CLI executable.
 */
public final class GrokCliResolver {

    private GrokCliResolver() {
    }

    /**
     * 成功路径缓存。CLI 是 one-shot 架构(每轮 send 新建 session),{@code findExecutable()} 每轮都被调用,
     * 而内部 {@link #verify(String)} 会 spawn 'grok --version' 子进程,
    ...
    private static String doFindExecutable() {
        ...
        if (PlatformUtils.isWindows()) {
            candidates.add(ProviderType.GROK.cliCommandWindows());
            candidates.add("grok.exe");
            candidates.add("grok.bat");
        } else {
            candidates.add(ProviderType.GROK.cliCommand());
        }
        ...
        return ProviderType.GROK.cliCommandForPlatform();
    }

    /**
     * 从 grok shim 路径推断 npm 全局结构下的原生二进制入口
     * ({@code <shim-dir>/node_modules/grok/bin/grok.exe})。纯路径逻辑,不验证可执行性。
     *
     * @param shimPath grok shim(grok.cmd/grok)的绝对或相对路径
    ...
    static String inferNativeExecutablePath(String shimPath) {
        ...
        File nativeExe = new File(shimDir,
                "node_modules" + File.separator + "grok" + File.separator + "bin" + File.separator + "grok.exe");
        return nativeExe.exists() ? nativeExe.getAbsolutePath() : null;
    }

    /**
     * 解析 grok 原生二进制入口(仅 Windows),绕过 .cmd/.bat 批处理包装。直接 spawn 原生 .exe
    ...
    private static String resolveNativeExecutable() {
        ...
        String shim = resolve(ProviderType.GROK.cliCommandWindows());
        if (shim == null) {
            shim = resolve("grok");
        }
    ...
    private static String searchInPath(String candidate) {
        ...
        // 修复 Windows 下经 npm 全局 / scoop / volta 装的 grok 在 IDE PATH 找不到 → fallback 裸名 → serve 启动失败。
        ...
        try {
            ...
            if (process.exitValue() == 0 && version != null) {
                ...
                if (!trimmed.isEmpty() && CliCompatibilityService.getInstance()
                        .isVersionAccepted(ProviderType.GROK, trimmed)) {
