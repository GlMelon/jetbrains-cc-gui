 ...
 * Pi 消息归一化器(透传实现)。
 * <p>
 * Pi 的事件解析在更底层完成——CLI 模式由 {@code PiCliSession}
 * 把 {@code pi run --format json} 的 marker 事件映射到统一 {@code MSG_*}。因此到达本归一化器时,
 ...
 * 路由,不区分同类)。
 */
public final class PiMessageNormalizer extends ForwardingMessageNormalizer {
    public PiMessageNormalizer(MessageCallback delegate) {
        super(delegate);
    }
}
