 ...
 * <p>
 * 按 ProviderType 单维路由到各 CLI SessionRuntime 实现(SDK 调用模式已移除,
 * runtime 维度已消除)。
...
public class SessionRuntimeRouter {
    ...
    public SessionRuntimeRouter(Project project) {
        ...
        // 注册全部 provider 的 CLI runtime 实现
    ...
    /**
     * 统一入口：按 provider 路由到对应 runtime 实现。
     */
    public CompletableFuture<CliResult> send(SessionRequest req, MessageCallback cb) {
        return registry.resolve(req.provider()).send(req, cb);
    }

    /**
     * 统一中断：按 provider 路由到对应 runtime 实现。
     */
    public void interrupt(ProviderType provider, String tabId) {
        registry.resolve(provider).interrupt(tabId);
    }
