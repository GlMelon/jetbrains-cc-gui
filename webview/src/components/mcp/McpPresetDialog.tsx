...
export function McpPresetDialog({ onClose, onSelect }: McpPresetDialogProps) {
  ...
  const getServerType = (preset: McpPreset): string => preset.server.type || 'stdio';

  const getServerTypeLabel = (preset: McpPreset): string => {
    const type = getServerType(preset);
    const labels: Record<string, string> = { stdio: 'STDIO', http: 'HTTP', sse: 'SSE' };
  ...
  return (
    <BaseDialog isOpen onClose={onClose} animation="pop">
      <DialogHeader title="选择预设服务器" onClose={onClose} />
      <DialogBody>
