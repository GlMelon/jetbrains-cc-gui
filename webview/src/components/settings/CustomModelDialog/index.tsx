...
export function CustomModelDialog({
  ...
  const buildModelFromForm = useCallback((): CodexCustomModel => {
    ...
    const contextWindow = parseContextWindowKInput(newContextWindowK);
    ...
    if (contextWindowEnabled && contextWindow !== undefined) {
  ...
  const handleEditModel = useCallback((model: CodexCustomModel) => {
    ...
    setNewContextWindowK(!contextWindowEnabled || model.contextWindow === undefined
      ? ''
      : String(model.contextWindow / CONTEXT_WINDOW_TOKENS_PER_K));
    setNewPricingInputs({
                ...
                models.map((model) => (
                  <div key={model.id} className={styles.modelItem} role="listitem">
                    <div className={styles.modelItemContent}>
                      ...
                      {contextWindowEnabled && model.contextWindow !== undefined && (
