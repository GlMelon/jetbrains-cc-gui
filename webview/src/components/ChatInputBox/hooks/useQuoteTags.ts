 ...
 *
 * Mirrors the file-tag mechanism (see {@link useFileTags}): {@link useTextContent#getTextContent}
 * serializes a `.quote-tag` chip back to its compact token (via `makeQuoteToken`), and this pass
 * rebuilds the chip from the token. The full Markdown blockquote is expanded from the registry
 * only at send time (see `expandQuoteTokens`).
 *
 * Cursor math relies on the token being a plain text node at save time: `getVirtualCursorPosition`
 * is called *before* the token text node is replaced by the chip, so its offset already accounts
 * for the full token length. After replacement `setVirtualCursorPosition` restores it; any residual
 * rounding at the chip boundary is imperceptible in practice (matches upstream behavior —
 * `virtualCursorUtils` intentionally does not special-case `.quote-tag`).
 */
  ...
  const renderQuoteTags = useCallback(() => {
    ...
    const walk = (node: Node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        // Skip into everything except already-rendered file tags (their text is decorative too).
