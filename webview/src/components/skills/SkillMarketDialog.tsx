...
function mapErrorMessage(
  ...
  switch (code) {
    case 'UNKNOWN_SOURCE': return t('skills.market.errorUnknownSource');
    case 'INVALID_SKILL_NAME': return t('skills.market.errorInvalidName');
    case 'HASH_MISMATCH': return t('skills.market.errorHashMismatch');
    case 'HTTP_404': return t('skills.market.errorNotFound');
    case 'HTTP_403': return t('skills.market.errorRateLimit');
    case 'NETWORK_ERROR': return t('skills.market.errorNetwork');
    case 'TIMEOUT': return t('skills.market.errorTimeout');
    case 'PARSE_ERROR': return t('skills.market.errorParse');
    case 'INSTALL_FAILED': return t('skills.market.errorInstall');
    default:
 ...
 */
export function SkillMarketDialog({ currentProvider, onClose, onInstalled }: SkillMarketDialogProps) {
  const { t } = useTranslation();
  const isCodex = currentProvider === 'codex';
  const primaryScope = isCodex ? 'user' : 'global';
  const secondaryScope = isCodex ? 'repo' : 'local';
  ...
  const [scope, setScope] = useState(primaryScope);
  ...
  const fetchSkills = useCallback(async (sourceId: string) => {
    setLoading(true);
    setError(null);
    const result: ListSkillMarketResult = await listSkillMarket(sourceId);
    setLoading(false);
    if (result.error) {
      setError({ message: mapErrorMessage(result.errorCode, result.error, t), code: result.errorCode });
      setSkills([]);
      return;
    }
  ...
  const handleSwitchSource = (sourceId: string) => {
    ...
    setScope(isCodex ? 'user' : 'global'); // 切源重置 scope 为 primary
    fetchSkills(sourceId);
  };

  const handleInstall = useCallback(async (skill: SkillMarketItem) => {
    ...
    if (result.success) {
      ...
      setToast({
        message: mapErrorMessage(result.errorCode, result.error || t('skills.market.errorInstall'), t),
        type: 'error',
      });
    }
  }, [activeSource, scope, onInstalled, t]);

  const handleShowDetail = useCallback(async (skill: SkillMarketItem) => {
  ...
  return (
    <BaseDialog isOpen onClose={onClose} size="lg" ariaLabel={t('skills.market.title')} className="skill-market-dialog">
      ...
      <DialogBody>
        ...
        {loading && (
          <div className="market-loading">
            <span className="codicon codicon-loading codicon-modifier-spin"></span> {t('skills.market.loading')}
          </div>
        )}
        ...
        {!loading && !error && skills.length > 0 && (
          <div className="result-meta">
            <span className="count">{t('skills.market.resultCount', { count: skills.length })}</span>
            <span>{t('skills.market.sourceLabel', { source: sources.find((s) => s.id === activeSource)?.label || activeSource })}</span>
          </div>
        )}
      ...
      {detailSkill && (
        <BaseDialog isOpen onClose={handleCloseDetail} size="lg" ariaLabel={t('skills.market.detail')} className="market-detail-dialog">
          <DialogHeader title={detail?.name || detailSkill.name} onClose={handleCloseDetail} />
          <DialogBody>
            {detailLoading && (
              <div className="market-loading">
                <span className="codicon codicon-loading codicon-modifier-spin"></span> {t('skills.market.detailLoading')}
              </div>
            )}
            {!detailLoading && detail?.error && (
              <div className="market-error">{mapErrorMessage(detail.errorCode, detail.error || t('skills.market.errorParse'), t)}</div>
            )}
              ...
              <div className="market-detail-content">
                ...
                <div className="detail-meta-grid">
                  ...
                  {detail.compatibility && (
                    <div className="detail-meta-item">
                      <span className="detail-meta-label">{t('skills.market.fieldCompatibility')}</span>
                      <span className="detail-meta-value">{detail.compatibility}</span>
                    </div>
                  )}
                  <div className="detail-meta-item">
                    <span className="detail-meta-label">{t('skills.market.fieldUserInvocable')}</span>
                    <span className="detail-meta-value">{detail.userInvocable ? t('skills.market.valueYes') : t('skills.market.valueNo')}</span>
                  </div>
                  {detail.allowedTools && (
                    <div className="detail-meta-item">
                      <span className="detail-meta-label">{t('skills.market.fieldAllowedTools')}</span>
                  ...
                  <section className="detail-section">
                    ...
                    <div className="detail-paths">
                      {detail.paths.map((p, i) => (
                        <code key={i} className="detail-path-tag">{p}</code>
                      ))}
                    </div>
                  </section>
                )}
          ...
          <DialogFooter>
            <button className="btn btn-secondary" onClick={handleCloseDetail}>{t('common.close')}</button>
            <button
              className="btn btn-primary"
              onClick={() => { handleCloseDetail(); handleInstall(detailSkill); }}
