...
// the only signal we can rely on across versions, because recent versions no
// longer guarantee a task_notification SDK event for background agents — the
// agent's terminal report is delivered solely as a main-session user-message XML
// (recovered by collectTaskEventsFromMessages). Matching this text is what keeps
// the card from flipping to "completed" the instant the ack lands.
 ...
 * `remote_launched` / `teammate_spawned` tool-use status and stamps the launch
 * ack with a fixed "Async agent launched" text. Both are accepted as fallbacks
 * so an agent whose input lacks `run_in_background` (e.g. spawned via a
 * different async path) is still recognized and is NOT marked completed on the
 * launch ack alone.
 */
 ...
 * undefined for any shape that does not carry one. Centralized so the
