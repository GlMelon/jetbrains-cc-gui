...
function getStructuralValueSignature(value: unknown): string {
  ...
  // missed structural change practically impossible. Must match the Java-side
  // StreamMessageCoalescer.computeMessageStructuralSignature dual hash.
  let hash = 0;
  let fnv = 0x811c9dc5;
...
export function registerMessageCallbacks(
  ...
  const processUpdateMessages = (json: string, sequence: number | null = null) => {
    ...
    try {
      ...
      const parsed = raw as ClaudeMessage[];
      if (sequence != null) {
  ...
  const processMessageTail = (
    ...
    try {
      const raw = JSON.parse(json) as unknown;
      if (!Array.isArray(raw)) return;
      const tail = raw as ClaudeMessage[];
      if (sequence != null) {
        window.__minAcceptedUpdateSequence = Math.max(minAcceptedSequence, sequence);
      }
      ...
      setMessages((prev) => {
        ...
        const currentBaseIndex = typeof storedBaseIndex === 'number'
          && Number.isSafeInteger(storedBaseIndex)
    ...
    if (isStreamingRef.current) {
      ...
      if (pendingUpdateRaf === null) {
        const timerId = setTimeout(() => {
          ...
          // Resume deferred delta rendering now that the structural snapshot
          // has established message identity in the React tree.
